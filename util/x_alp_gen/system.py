...
from .memory.ram_bank import Bank, is_pow2, ILRamGroup
from .memory.linker_section import LinkerSection
...
class Override:
    """
    Bundles information that can be overriden in the XAlp class.
    """
...
class XAlp: 
    ...
    def __init__(
        ...
        if not type(bus_type) is BusType:
            raise TypeError(
                f"XAlp.bus_type should be of type BusType not {type(self._bus_type)}"
            )
    ...
    def validate(self) -> bool:
        ...
        This should be called before using the XAlp object to generate the project.
