...
// e41cfc3001293b5625c25412bd9b26e6e4ab8f7e)
//
...
static void write(uint64_t address, uint64_t len, uint8_t *buf) {
  ...
  for (int i = 0; i < len; i++) { mem.push_back(buf[i]); }
...
extern "C" char read_section(long long address, const svOpenArrayHandle buffer, long long len) {
  ...
  for (auto &datum : mems.find(address)->second) {
    if (len_tmp-- == 0) {
      printf("[ELF] ERROR: Copied 0x%lx bytes. Buffer is full but there is "
...
extern "C" char read_section_chunk(long long address, long long offset, char *buffer, long long len) {
  ...
  if (offset < 0 || len < 0 || ((offset + len) > mem.size())) {
    printf("[ELF] ERROR: Offset %0p, length %d out of bounds for section at %p "
           "with length %d\n",
...
template <class E, class P, class Sh, class Sy> static void load_elf(char *buf, size_t size) {
  E *eh = (E *)buf;
  ...
  if (size < eh->e_phoff + (eh->e_phnum * sizeof(P))) {
    printf("[ELF] ERROR: Filesize is smaller than advertised program headers "
           "(0x%lx vs 0x%lx)\n",
  ...
  for (unsigned int i = 0; i < eh->e_phnum; i++) {
    // Check whether the current program header entry contains a loadable
    // section of nonzero size
    if (ph[i].p_type == PT_LOAD && ph[i].p_memsz) {
      ...
      if (ph[i].p_memsz > ph[i].p_filesz) {
        printf("[ELF] WARNING: The section starting @ %p contains 0x%lx zero "
               "bytes which will NOT be preloaded!\n",
  ...
  if (size < eh->e_shoff + (eh->e_shnum * sizeof(Sh))) {
    printf("[ELF] ERROR: Filesize is smaller than advertised section headers "
           "(0x%lx vs 0x%lx)\n",
  ...
  if (eh->e_shstrndx >= eh->e_shnum) {
    printf("[ELF] ERROR: Malformed ELF file. The index of the section header "
           "strings is out of bounds (0x%lx vs max 0x%lx)",
           eh->e_shstrndx, eh->e_shnum);
    return;
  }

  if (size < sh[eh->e_shstrndx].sh_offset + sh[eh->e_shstrndx].sh_size) {
    printf("[ELF] ERROR: Filesize is smaller than advertised size of section "
           "name table (0x%lx vs 0x%lx)\n",
  ...
  for (unsigned int i = 0; i < eh->e_shnum; i++) {
    // Get an upper limit on how long the name can be (length of the section
    // name section minus the offset of the name)
...
extern "C" char read_elf(const char *filename) {
  ...
  if (size < sizeof(Elf64_Ehdr)) {
    printf("[ELF] ERROR: File %s is too small to contain a valid ELF header "
           "(0x%lx vs 0x%lx)\n",
